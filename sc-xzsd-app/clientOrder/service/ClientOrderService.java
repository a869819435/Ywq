...
public class ClientOrderService {
    ...
    public AppResponse addOrder(ClientOrderGoods clientOrderGoods,String shopCartId){
        ...
        //获取选中的商品库存
        List<Integer> goodsInventorys = clientOrderDao.getGoodsInventory(goodsIds);
        //记录所有商品购买后的剩余库存
        ...
        for (int i = 0 ; i < goodsInventorys.size() ;i++ ){
            int ans = goodsInventorys.get(i) - Integer.valueOf(clientGoodsNums.get(i));
            if( ans < 0){
    ...
    public AppResponse listOrder(String orderStateId){
        ...
        List<ClientOrderVO> clientOrderVOList = clientOrderDao.listOrderByPage(userId,orderStateId);
        ...
        int sum = clientOrderVOList.size();
        for (int i = 0 ; i < sum ; i++ ){
            String key = clientOrderVOList.get(i).getOrderId();
            //获取订单的id对应的value，初始化订单商品列表
            map.put(key,i );
            clientOrderVOList.get(i).setGoodsList(new ArrayList<>());
            orderIds.add(clientOrderVOList.get(i).getOrderId());
        }
        ...
        clientOrderGoodsVOList.forEach(temp -> {
            ...
            clientOrderVOList.get(parent).getGoodsList().add(temp);
        });
        return AppResponse.success("查询订单列表成功",getPageInfo(clientOrderVOList));
    }
    ...
    public AppResponse listOrderDeepen(String orderId){
        ...
        return AppResponse.success("获取订单详情成功！",clientOrderDeepenVO);
    }
    ...
    public AppResponse listGoodsForEvaluate(String orderId){
        //获取订单商品的图片
        List<ClientOrderGoods> clientOrderGoodsList = clientOrderDao.listGoodsForEvaluate(orderId);
        if(clientOrderGoodsList == null){
            return AppResponse.notFound("获取订单商品信息失败");
        }
        return AppResponse.success("获取订单商品信息成功！",clientOrderGoodsList);
    }
